...
The implemented editor is intentionally narrower than the vision. Version 0.3.0 provides one inventory-approved Surge XT instrument, one looping clip, a piano roll, tempo and loop controls, exact plug-in-state persistence, real-time WASAPI playback, native Surge editing and audition controls, project Undo/Redo, and the accepted M4 snapshot-first A/B workflow for explicitly accepting one named sound. Factory-preset indexing, multiple tracks, arrangement, automation, AI commands, game-state logic, effects, and final exports remain roadmap work. M5's unified validated edit-command layer is the next milestone.
