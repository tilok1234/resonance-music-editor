...
The implemented editor is intentionally narrower than the vision. Version 0.5.0 exposes and persists one through four ordered inventory-approved Surge XT instances, each with one clip on a canvas of one through 64 bars, exact accepted plug-in state, gain, pan, mute, solo, MIDI routing, and selected-track editing. It retains real-time WASAPI playback, native Surge editing, the accepted M4 sound A/B workflow, and the accepted M5 validated note-proposal workflow.

Authoring throughput was widened on 2026-08-12 and 2026-08-13: external version-1 edit-command files load into the accepted preview path, the piano roll gained vertical and horizontal zoom with ghost notes for inactive tracks, notes support multiple selection and a clipboard, sound work persists in a named shelf outside the song, and song-project schema version 5 migrates versions 1 through 4 without source rewrite.

A user-approved audible mix, user-facing missing-plug-in recovery, different instrument products per track, factory-preset indexing, named sections, reusable clip instances, tempo and meter changes, automation, natural-language AI translation, game-state logic, effects, and final exports remain roadmap work.

The first user listening pass, on 2026-08-13, found a mix defect that every silent gate had passed. That is the clearest evidence so far for the principle above: technical and musical gates remain separate, and only the second one can find that class of problem.
