...
The implemented editor is intentionally narrower than the vision. Version 0.5.0 exposes and persists one or two ordered inventory-approved Surge XT instances, each with one looping clip, exact accepted plug-in state, gain, pan, mute, solo, MIDI routing, and selected-track editing. It retains real-time WASAPI playback, native Surge editing, the accepted M4 sound A/B workflow, and the accepted M5 validated note-proposal workflow. M6 now has schema-version-3 migration from both prior formats, stable project-wide identity, a fixed eight-lane realtime ownership contract, and technically verified bounded two-track authoring. A user-approved audible mix, user-facing missing-plug-in recovery, different instrument products, factory-preset indexing, arrangement, automation, natural-language AI translation, game-state logic, effects, and final exports remain roadmap work.
