...
The implemented editor is intentionally narrower than the vision. Version 0.4.0 still exposes and persists one inventory-approved Surge XT instrument and one looping clip, with a piano roll, tempo and loop controls, exact plug-in-state persistence, real-time WASAPI playback, native Surge editing, the accepted M4 sound A/B workflow, and the accepted M5 validated note-proposal workflow. M6 adds schema-version-2 migration, stable track/clip identity, persisted mixer/MIDI settings, a fixed eight-lane realtime ownership contract, and a silent technical gate that renders two distinct real Surge instances through the production mixer. Visible/persisted multi-track authoring, a user-approved audible mix, factory-preset indexing, arrangement, automation, natural-language AI translation, game-state logic, effects, and final exports remain roadmap work.
